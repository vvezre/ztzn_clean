...
def normalize_loop_options(payload, waypoint_count):
    ...
    if not loop_enabled:
        return {'loop': False, 'loopMode': 'count', 'loopCount': 1}
    if waypoint_count < 2:
        raise ValueError('闭环循环至少需要2个路点')
    if loop_mode not in ('count', 'continuous'):
        raise ValueError('循环方式无效')
    if loop_mode == 'continuous':
        return {'loop': True, 'loopMode': 'continuous', 'loopCount': 0}
    ...
    try:
        ...
        loop_count = 0
    if loop_count <= 0:
        raise ValueError('循环圈数必须大于0')
    return {'loop': True, 'loopMode': 'count', 'loopCount': loop_count}
...
def iter_closed_loop_targets(waypoints, loop_count=None):
    ...
    yield {'index': 0, 'waypoint': waypoints[0], 'completed_loop': 0}
    cycle = 0
    while loop_count is None or cycle < loop_count:
        ...
        cycle += 1
        yield {'index': 0, 'waypoint': waypoints[0], 'completed_loop': cycle}
