...
The return command must not draw a new straight line across cleaning areas.
Instead, this module treats every saved executable segment (including bridge
segments) as an undirected graph and finds the shortest path back to the task
origin.  The vehicle therefore only travels over geometry that was already
approved by the route planner.
...
def _build_graph(tasks):
    ...
    for task in tasks:
        ...
        graph.setdefault(start_key, []).append((end_key, distance, task))
        graph.setdefault(end_key, []).append((start_key, distance, task))
    return nodes, graph
...
def build_return_to_origin_tasks(task_config, current_lat, current_lon,
                                 active_segment=None, max_idle_join_cm=MAX_IDLE_JOIN_CM):
    ...
    position to the segment that the robot is actually executing.  Otherwise
    the robot may only join the saved route at a nearby node; a distant direct
    line is rejected because it could cross an obstacle.
    """
    ...
    active_anchor = bool(candidates)
    if not candidates:
        # An idle vehicle must first join the nearest saved route node.  Do not
        # let the total-cost comparison prefer a farther node (especially the
        # origin itself), because that would recreate the unsafe direct line
        # across the work area that this planner is intended to prevent.
        nearest_join = min(
            _distance_cm(current, nodes[node_key])
            for node_key in distances.keys()
        )
        candidates = [
            node_key for node_key in distances.keys()
            if _distance_cm(current, nodes[node_key]) <= nearest_join + 1.0
        ]
    if not candidates:
    ...
    chosen = min(
        candidates,
        key=lambda node_key: _distance_cm(current, nodes[node_key]) + distances[node_key],
    )
    join_distance = _distance_cm(current, nodes[chosen])
    if not active_anchor and join_distance > float(max_idle_join_cm):
        raise ReturnOriginRouteError(
            "RETURN_POSITION_OFF_ROUTE",
            "当前位置距离已知路线过远，不能安全自动回原点",
        )
