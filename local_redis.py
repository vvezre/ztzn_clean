...
def _is_tcp_reachable(host, port, timeout=0.2):
    try:
...
def create_redis_client(host='localhost', port=6379, db=0, decode_responses=True, local_mode=False,
                        ...
                        fallback_if_unavailable=False, probe_timeout=0.2):
    if local_mode:
        return _LOCAL_CLIENT
    if fallback_if_unavailable and not _is_tcp_reachable(host, port, probe_timeout):
        return _LOCAL_CLIENT
    return redis.Redis(
        host=host,
        port=port,
